...
6. **Mid-loop cancel detection** (only relevant for `eOtherFunction` / `eOtherFunctionAll` types that iterate over many lines or files internally): if you want the user to be able to cancel quickly during your function's own inner loop, use `Get pbUserCancelRequested of ghoEngine to bCancel` and exit your local loop when it returns True. **Never call `Get Cancel_Check of ghoEngine`** — that's the engine's cancel pipeline (it deactivates the status panel and pops the YesNo) and calling it from inside a refactor function silently consumes the cancel signal, leaving the engine grinding through the rest of the workspace while the program appears locked up. See `cRefactorEngine.pkg`'s `pbUserCancelRequested` for the rationale, and `UserDefinedRefactorFunctions.pkg` for the canonical code pattern.
